...
@Service
public class WorkflowService implements BiConsumer<WorkflowModel, String>, Function<String, IWorkflow>, Supplier<Set<IWorkflow>> {
	...
	public void accept(WorkflowModel workflowModel, String instanceId) {
		if (executingWorkflows.containsKey(instanceId))
			throw new RuntimeException(MessageFormat.format("A workflow instance with id {0} already exists!", instanceId));

		// Convert the workflow to an executable IWorkflow object
		IWorkflow convertedWorkflow = this.workflowConversionService.convertWorkflowModel(workflowModel);
	...
	public IWorkflow apply(String instanceId) {
		if (executingWorkflows.containsKey(instanceId)) {
			...
			throw new RuntimeException(MessageFormat.format("Workflow instance {0} could not be found!", instanceId));
		}
	}
