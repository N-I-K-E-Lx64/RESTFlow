...
public class RamlParserService implements BiFunction<String, String, Api> {
	...
	public ApiStorage parseRaml(File ramlFile, String modelId) {
		...
		if (ramlModelResult.hasErrors()) {
			ramlModelResult.getValidationResults().forEach(validationResult -> logger.error(validationResult.getMessage()));
			throw new RuntimeException(MessageFormat.format("Raml file {0} contains errors!", ramlFile.getName()));
		} else {
			...
			if (!Objects.isNull(parsedApi)) {
				final ApiStorage apiStorage = new ApiStorage(ramlFile.getName(), parsedApi, this.extractResources(parsedApi));
	...
	/**
	 * Returns a list of all ApiStorage-objects related to a specific model. If the state does not have an entry for that model,
	 * but the corresponding folder is not empty, these api files are being parsed and the resulting ApiStorage-objects are returned.
	 * @param modelId Id of the corresponding model (name of the folder)
	...
	public List<ApiStorage> getRamlFilesForModel(String modelId) {
		if (ramlFiles.containsKey(modelId)) {
			...
			if (!storedFiles.isEmpty()) {
				...
				throw new RuntimeException(MessageFormat.format("No api description could be found for model {0}!", modelId));
			}
		}
	}
}
